...
export function SystemAdminPage() {
  const auth = useAuth();
  const canReadSessions = auth.hasPermission("users.read");
  const control = useQuery({ queryKey: ["admin-system"], queryFn: controlApi.system, refetchInterval: 10_000 });
  const sessions = useQuery({ queryKey: ["admin-sessions"], queryFn: controlApi.sessions, refetchInterval: 30_000, enabled: canReadSessions });
  const users = useQuery({ queryKey: ["admin-users", "system"], queryFn: controlApi.users, enabled: canReadSessions });
  ...
  return <>
    <Topbar title="Saúde do sistema" subtitle="Control plane e operação" onRefresh={() => { void control.refetch(); void sessions.refetch(); void monitor.refetch(); }} refreshing={control.isFetching || sessions.isFetching || monitor.isFetching || users.isFetching}/>
    <div className="content-grid">
      ...
      <div className="two-col"><Panel title="Sessões"><div className="metric-grid"><div className="metric-cell"><span>Ativas</span><strong>{canReadSessions ? sessions.data?.length ?? "—" : "restrito"}</strong></div><div className="metric-cell"><span>TTL</span><strong>{control.data ? `${control.data.sessionTtlSeconds / 3600} h` : "N/D"}</strong></div><div className="metric-cell"><span>Cookie Secure</span><strong>{control.data ? String(control.data.cookieSecure) : "N/D"}</strong></div><div className="metric-cell"><span>Gateway sessions</span><strong>{gateway?.activeSessions ?? "N/D"}</strong></div></div>{canReadSessions ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Usuário</th><th>Expira</th><th>Atual</th></tr></thead><tbody>{(sessions.data ?? []).map((session,index)=><tr key={`${session.userId}-${session.expiresAt}-${index}`}><td>{userNames.get(session.userId) ?? session.userId}</td><td>{formatDateTime(session.expiresAt)}</td><td>{session.current ? "SIM" : "NÃO"}</td></tr>)}</tbody></table></div> : <p className="panel-note">Detalhes de sessões exigem users.read.</p>}</Panel><Panel title="Provider"><p>{monitor.data?.provider.message || "N/D"}</p><small>Última verificação: {formatDateTime(monitor.data?.provider.checkedAt)}</small></Panel></div>
