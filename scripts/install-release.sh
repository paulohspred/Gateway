...
ROOT="/opt/rc-gateway"
CONFIG_TARGET="/etc/rc-gateway.json"
UNIT_TARGET="/etc/systemd/system/rc-gateway.service"
...
if tar -tzf "$ARCHIVE" | grep -Eq '(^/|(^|/)\.\.(/|$))'; then
  echo "ERRO: release contém caminho inseguro." >&2
  exit 4
fi
...
if [[ -e /opt/rc-gateway-umbrella || -e /etc/rc-gateway-umbrella.json || -e /etc/systemd/system/rc-gateway-umbrella.service ]]; then
  echo "ERRO: instalação legada rc-gateway-umbrella detectada. Faça migração explícita antes de instalar a identidade canônica rc-gateway." >&2
  exit 6
fi

getent group rc-gateway >/dev/null || groupadd --system rc-gateway
if ! id rc-gateway >/dev/null 2>&1; then
  useradd --system --gid rc-gateway --home-dir /nonexistent --shell /usr/sbin/nologin rc-gateway
fi
if getent group dialout >/dev/null; then
  usermod -a -G dialout rc-gateway
fi
...
install -o root -g root -m 0644 "$release_dir/systemd/rc-gateway.service" "$UNIT_TARGET"
...
rollback(){
  ...
  if [[ $had_config -eq 1 && -n "$config_backup" && -f "$config_backup" ]]; then
    cp --preserve=mode,ownership,timestamps "$config_backup" "$CONFIG_TARGET"
...
echo "INSTALL OK: release=$version current=$release_dir health=$HEALTH_URL frontend=packaged monitor=executable"
