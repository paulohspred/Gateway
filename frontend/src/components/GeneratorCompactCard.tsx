...
function text(row: FleetRow, key: string, digits = 1) { const v = metricView(row.telemetry, key, { digits }); return !v.present || ["N/D","Dado inválido"].includes(v.display) ? v.display : `${v.display}${v.unit ? ` ${v.unit}` : ""}`; }

export function GeneratorCompactCard({ row }: { row: FleetRow }) {
  const mode = metricView(row.telemetry, "controller.mode");
